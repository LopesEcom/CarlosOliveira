 ...
 *   public/icon.png                favicon, 512 px, transparente
...
async function pintar({ alfa, largura, altura }, cor, destino) {
  ...
  await sharp(rgba, { raw: { width: largura, height: altura, channels: 4 } }).png({ compressionLevel: 9 }).toFile(em(destino))
  console.log(destino, `${largura}x${altura}`)
}

const logo = await mascara('marca/original/logo.png', 1200)
...
await pintar(simbolo, TINTA, 'public/marca/simbolo-tinta.png')
await pintar(simbolo, CREME, 'public/marca/simbolo-creme.png')

// Favicon quadrado com respiro: o símbolo é redondo e encosta na borda.
const quadrado = async (lado, respiro, fundo, destino) => {
  const miolo = await sharp(em('public/marca/simbolo-tinta.png')).resize(lado - respiro * 2, lado - respiro * 2, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } }).toBuffer()
  await sharp({ create: { width: lado, height: lado, channels: 4, background: fundo } }).composite([{ input: miolo }]).png().toFile(em(destino))
  console.log(destino)
}
await quadrado(512, 16, { r: 0, g: 0, b: 0, alpha: 0 }, 'public/icon.png')
