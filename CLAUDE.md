...
- **Logo**: o brasão **Artepira** que o genro do Carlos fez (pedido dele), original em `marca/original/logo-artepira.png`. `node scripts/marca.mjs` gera `public/marca/logo.png` (colorida, para o branco), `public/marca/logo-contraste.png` (em contraste, numa cor só, para a foto da capa e o rodapé), `public/icon.png` e `apple-touch-icon.png`. No cabeçalho as duas trocam por opacidade ao rolar. O arquivo é pequeno (382 × 290): não exibir acima disso. O selo de anéis anterior ficou em `marca/antigo/`. O brasão diz "desde 1966" (60 anos) e o site diz 58 anos de bancada: diferença a confirmar com o Carlos.
- **Cabeçalho** (pedido do Edson, referência "Oaklume", sem a sacola): grade de três colunas `minmax(0,1fr) auto minmax(0,1fr)`. No celular: menu à esquerda, logo no centro exato, lupa à direita. No computador: logo à esquerda, links centralizados em Inter média sem caixa alta (com "Início"), lupa à direita. A lupa abre um campo que leva a `/acervo?q=`.
- **Imagens e carregamento**: `next.config.ts` serve AVIF/WebP com larguras curtas (`deviceSizes`/`imageSizes`) e cache de 30 dias. Em `<Image>` de tamanho fixo (logo, miniaturas), `width`/`height` são o tamanho exibido, não o do arquivo, senão o navegador baixa a versão de 3840 px. A segunda foto do cartão da grade só monta no hover de mouse. Os vídeos respeitam economia de dados (`Save-Data`/2G: pôster e play). A serifa carrega só o peso 400. Para medir peso de verdade use a versão de produção (`preview_start` "producao", porta 3100, depois de `npm run build`).
- **Livro = apostila Arte Pira** (pedido do Edson): `brand.livro` é a apostila (capa `public/livro/arte-pira.webp`, `link` para o material, usado também por `CAMINHO_APRENDER`). O Arterapia fica em `brand.autobiografia` só para atribuir as falas da História. Não descreva o conteúdo da apostila além do que está na capa.
- **Fotos do Carlos: no máximo duas no site** (pedido do Edson): hoje só a do topo da página História (`public/atelier/Foto-Carlos.jpeg`). A capa do início usa a foto do busto de Cristo (`public/capa/cristo.webp`, a mesma da og-image), sem o Carlos.
