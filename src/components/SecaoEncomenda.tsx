...
const PASSOS = [
  ...
  { titulo: 'A madeira', texto: 'O Carlos escolhe o bloco que aguenta o desenho e diz o prazo real.' },
  { titulo: 'O entalhe', texto: 'A peça é aberta à goiva, do desbaste ao acabamento, sem emenda.' },
]
...
export default function SecaoEncomenda() {
  return (
    <section className="border-t border-borda-sutil bg-creme">
          ...
          <ol className="col-6 deslocar-7 mt-8 grid gap-10 self-end md:mt-0">
            {PASSOS.map((passo, indice) => (
              <Revelar key={passo.titulo} como="li" atraso={indice * 110} className="grid grid-cols-[4.5rem_1fr] gap-4 border-t border-borda pt-6">
                <span className="font-display text-[2.75rem] font-light leading-none text-tinta/40">
                  {String(indice + 1).padStart(2, '0')}
                </span>
                <div>
                  <h3 className="text-h5 uppercase tracking-largo">{passo.titulo}</h3>
